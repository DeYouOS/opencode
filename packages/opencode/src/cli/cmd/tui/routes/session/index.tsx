    ...
    <>
      ...
      <box paddingLeft={3}>
        <text marginTop={1}>
          ...
          <Show
            when={
              sync.data.provider.find((p) => p.id === props.message.providerID)?.models?.[props.message.modelID]?.name
            }
            fallback={props.message.modelID}
          >
            {(name) => (
              <>
                <span style={{ fg: theme.text }}>{Locale.titlecase(props.message.mode)}</span>
                <span style={{ fg: theme.textMuted }}> · {name()}</span>
              </>
            )}
          </Show>
          <Show when={duration()}>
