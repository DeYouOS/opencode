...
const AUTHORIZE_URL = process.env.ANTHROPIC_AUTHORIZE_URL || "https://claude.ai/oauth/authorize"
const TOKEN_URL = process.env.ANTHROPIC_TOKEN_URL || "https://platform.claude.com/v1/oauth/token"
const REDIRECT_URI = process.env.ANTHROPIC_REDIRECT_URI || "https://platform.claude.com/oauth/code/callback"
const SCOPES =
  process.env.ANTHROPIC_SCOPES ||
  "user:profile user:inference user:sessions:claude_code user:mcp_servers user:file_upload"

const CLI_VERSION = process.env.ANTHROPIC_CLI_VERSION || "2.1.81"
const USER_AGENT = process.env.ANTHROPIC_USER_AGENT || `claude-cli/${CLI_VERSION} (external, cli)`
const BETA_FLAGS =
...
export async function exchangeCodeForTokens(rawCode: string, verifier: string): Promise<OAuthTokens> {
...
export async function refreshTokens(refreshToken: string): Promise<OAuthTokens> {
  ...
  if (!res.ok) {
    const text = await res.text().catch(() => "")
    throw new Error(`Token refresh failed: ${res.status} ${res.statusText}${text ? ` — ${text}` : ""}`)
  }
