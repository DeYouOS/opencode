...
const plugin: Plugin = async ({ client }) => {
  ...
  try {
    const config = await Bun.file(`${process.env.HOME}/.config/opencode/opencode.json`).json()
    const proxy = config?.provider?.anthropic?.options?.proxy
      ...
      async loader(getAuth, provider) {
        ...
        return {
          ...
          async fetch(input: RequestInfo | URL, init?: RequestInit) {
            ...
            if (typeof body === "string" && url.includes("/v1/messages")) {
              try {
                ...
                if (Array.isArray(parsed.system)) {
                  const hasIdentity = parsed.system.some((s: any) =>
                    typeof s === "string" ? s.includes(SYSTEM_IDENTITY) : s?.text?.includes(SYSTEM_IDENTITY),
                  )
                  if (!hasIdentity) {
            ...
            const proxy = _proxy ?? (init as any)?.proxy
